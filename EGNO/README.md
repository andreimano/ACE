...
This code is adapted from the original EGNO repository.

- In this ACE repository, the current implementation is tested only for the CMU MoCap setup (`main_mocap_no.py`).
- To switch datasets/settings, update the MoCap config file used by `--config_by_file` (default: `./configs/config_mocap_no.json`).
- The constrained training setup supports both equality constraints and inequality constraints.

Example run (resilience enabled):

```bash
python main_mocap_no.py --config_by_file --outf ./out --disable_equi 1 --use_constrained 1 --lr_dual 0.0002 --log_transform_variance 1  --use_wandb 1 --resilience 1 --num_runs 5
```
