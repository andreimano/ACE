...
def save_model(model, dir, id, constrained, gpu=""):
    """ Save a model """
    os.makedirs(dir, exist_ok=True)
    if gpu != "":
        gpu = "_" + str(gpu)

    torch.save(model.state_dict(), os.path.join(dir, id + gpu + f'constrained{constrained}' + ".pt"))


def load_model(model, dir, id, constrained, gpu=""):
    """ Load a state dict into a model """
    if gpu != "":
        gpu = "_" + str(gpu)
    state_dict = torch.load(os.path.join(dir, id + gpu + f'constrained{constrained}' + ".pt"))
