## ACE Adaptation Notes

This directory contains the ACE adaptation of the original VNN codebase.

- In this repository, we focus on the classification setup with VN-DGCNN on ModelNet. PointNet is not functional.

### Run Templates (Placeholders)

1. ACE run example:
```bash
python train_cls.py --model vn_dgcnn_cls --rot so3 --log_dir vn_dgcnn/aligned/ --use_wandb 1 --dual_lr 1e-4 --use_constrained 1 --disable_equivariance 1 --seed 1 --num_point 300 --dataset_dropout 0
```

2. Dropout experiment run:
```bash
python train_cls.py --model vn_dgcnn_cls --rot so3 --log_dir vn_dgcnn/aligned/ --use_wandb 1 --dual_lr 1e-4 --use_constrained 1 --disable_equivariance 1 --seed 1 --num_point 300 --dataset_dropout 1
```
